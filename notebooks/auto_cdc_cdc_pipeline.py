...
# acc-connector — Bronze AUTO CDC pipeline (CDC-beta delta feeds).
#
# Reads CDC CSV drops from
#   /Volumes/<catalog>/bronze/acc_bronze_volume/data_connector_cdc/<project>/<run>/*.csv
# sync_service passes ``acc.dc_cdc_path`` for one run folder. Applies
# ``create_auto_cdc_flow`` (not FROM SNAPSHOT) into the same Bronze targets
# as the snapshot pipeline. Deletes via ``deleted_at``; sequence via
# ``adsk_updated_at`` (fallback ``updated_at``).
#
...
# Set per sync by sync_service (Phase 3), e.g.
#   .../data_connector_cdc/<project_id>/2026-05-27T12-00-00
...
def _resolve_cdc_base() -> str:
    """Directory containing this sync's CDC CSV drop (one run folder)."""
    if DC_CDC_PATH:
        return DC_CDC_PATH.rstrip('/')

    try:
        if DC_PROJECT_ID:
            project_dirs = [f'{DC_CDC_BASE}/{DC_PROJECT_ID}']
        else:
            project_dirs = [e.path for e in dbutils.fs.ls(DC_CDC_BASE) if e.isDir()]
    except Exception:
        return ''
    ...
    if latest_path:
        _emit('INFO', f'CDC base (latest run): {latest_path}')
    return latest_path


def _discover_csv_basenames(cdc_base: str) -> list[str]:
    """Return CSV file names in ``cdc_base`` (one DC export folder)."""
    if not cdc_base:
        return []
    seen: set[str] = set()
    try:
        entries = dbutils.fs.ls(cdc_base)
    except Exception:
        return []
...
def _register_table(
    ...
    """Per-CSV: gate on PK + sequence column, build reconciled schema,
    ...
    if not cdc_base:
        ...
        _emit('ERROR', f'{sch}.{tbl}: skipped — no CDC directory resolved')
        _record_status(sch, tbl, STATUS_SKIPPED_NO_CSV, csv_header=[],
                       error_msg='acc.dc_cdc_path unset and no run folder found')
        _run_counts[STATUS_SKIPPED_NO_CSV] += 1
        return
    glob_path = f'{cdc_base}/{csv_name}'
    ...
    # 5. Sequence column required for AUTO CDC (not FROM SNAPSHOT).
    if 'adsk_updated_at' in header_set:
        sequence_col = 'adsk_updated_at'
    elif 'updated_at' in header_set:
        sequence_col = 'updated_at'
    else:
        _emit('ERROR',
              f'{sch}.{tbl}: skipped — no adsk_updated_at or updated_at in CSV header')
        _record_status(sch, tbl, STATUS_CSV_UNREADABLE, csv_header=header,
                       error_msg='missing sequence column for AUTO CDC')
        _run_counts[STATUS_CSV_UNREADABLE] += 1
        return

    # 6. Register the AUTO CDC flow with reconciled schema + DLT expectations.
    ...
    dlt.create_streaming_table(
        ...
        comment=f'ACC DC CDC beta -> SCD2 Bronze ({csv_name}). PK: {pk_columns}',
    )
    flow_kw = dict(
        target=table_name,
        source=view_name,
    ...
    if 'deleted_at' in header_set:
        flow_kw['apply_as_deletes'] = F.expr('deleted_at IS NOT NULL')
    dlt.create_auto_cdc_flow(**flow_kw)

    _record_status(sch, tbl, STATUS_OK, csv_header=header, column_diff=column_diff)
    _run_counts[STATUS_OK] += 1
    _emit('OK', f'{sch}.{tbl}: registered AUTO CDC flow, keys={pk_columns}, seq={sequence_col}')
...
def _bootstrap_schema_json() -> str | None:
    ...
    zip_paths = []
    if cdc_base:
        zip_paths.append(f'{cdc_base}/{_DC_EXTRACT_ZIP_NAME}')
        try:
            parent = '/'.join(cdc_base.rstrip('/').split('/')[:-1])
    ...
    schemas_dir = f'{cdc_base}/schemas' if cdc_base else ''
...
for _csv in _discover_csv_basenames(_cdc_base):
    ...
    _register_table(
        _csv, _registry_flat, _known_schemas, _cdc_base,
